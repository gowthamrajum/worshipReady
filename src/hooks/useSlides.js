...
const useSlides = () => {
  ...
  const duplicateSlide = (index, callback) => {
    ...
    // Append the duplicate as the last slide.
    setSlides((prev) => [...prev, copy]);
    const newIndex = slides.length; // will be the last index after append
  ...
  const setSlideBackground = (index, { backgroundColor, backgroundImage } = {}) => {
    setSlides((prev) =>
      prev.map((slide, i) => {
  ...
  const resetAllBackgrounds = () => {
    setSlides((prev) =>
      prev.map((slide) => ({ ...slide, backgroundColor: "#4b5c47", backgroundImage: null }))
    );
  };
