 ...
 */
async function renderSlideToImage(lines = [], bgColor = CANVAS_BG_CSS, bgImage = null) {
  ...
  if (bgImage) {
    ...
    } catch { /* image load failed — continue with color only */ }
  }
  ...
  try {
    ...
    for (const slide of localSlides) {
      ...
      if (!slide.lines || slide.lines.length === 0) {
        ...
        slideObj.background = { color: (slide.backgroundColor || CANVAS_BG_CSS).replace("#", "") };
        continue;
      }

      // Render the slide lines to an image and embed it — this preserves
      // exact font, Telugu script, colours, spacing, and centering.
      const imgData = await renderSlideToImage(slide.lines, slide.backgroundColor || CANVAS_BG_CSS, slide.backgroundImage);
