...
const AddSong = ({ onSongAdded }) => {
  ...
  const handleSubmit = async () => {
    ...
    if (RECAPTCHA_SITE_KEY && !captchaToken) {
      setModalState({ type: "error", message: "⚠️ Please complete the CAPTCHA first." });
      return;
    }
  ...
  return (
    <div className="bg-white p-6 rounded shadow-md space-y-6">
      ...
      <button
        ...
        className="mt-4 w-full bg-green-600 hover:bg-green-700 text-white py-2 rounded disabled:opacity-40"
      >
        Submit Song
      </button>
