...
const SongPreview = ({ dragMode, onAddMultipleSlides, onUndoLastBatch }) => {
  ...
  // ── Song list (debounced server-side search) ────────────────────────────────
  ...
  const fetchLyrics = async (song) => {
    try {
      const res = await fetchSong(song.song_id);
  ...
  // Songs are already filtered server-side
          ...
          <div className="space-y-3">
            ...
            {mode === "drag" && (
              <div className="space-y-2">
                ...
                {lyrics.map((stanza) => {
                  const allLines = [...stanza.telugu, ...stanza.english];
                  const normalize = (s) => s.replace(/\s{3,}\|\|/g, "  ||");
                  return (
                    <div
                      key={stanza.id}
                            ...
                            {stanza.telugu.map((line, j) => (
                              <div
                                ...
                                onDragStart={(e) =>
                                  e.dataTransfer.setData(
                                    "text/plain",
                                    JSON.stringify({ type: "line", text: normalize(line) })
                                  )
                                }
