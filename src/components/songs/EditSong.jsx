...
import { fetchSongList, fetchSong, updateSong } from "../../api/client";
...
const [languageEditorMinimized, setLanguageEditorMinimized] = useState(false);
  ...
  // Debounced search for song list (standalone mode)
  useEffect(() => {
    if (songId) return;
  ...
  useEffect(() => {
    if (songId || !debouncedQuery) return;
    fetchSongList(1, 50, debouncedQuery)
      .then(res => setSongs(res.data.songs))
      .catch(err => console.error("Error fetching songs:", err));
  }, [debouncedQuery, songId]);
  ...
  const saveSong = async () => {
    ...
    try {
      ...
      await updateSong(selectedSong.song_id, updatedLyrics);
