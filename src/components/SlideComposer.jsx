...
import WorkflowPsalmModal from "./WorkflowPsalmModal";
import useWorshipWorkflow from "../hooks/useWorshipWorkflow";
...
const SlideComposer = () => {
  ...
  // ── Offerings slide builder (shared with workflow) ──
  const addOfferingsSlideAtEnd = () => {
    ...
    addMultipleSlidesAtEnd([slideLines]);
  };

  // ── Worship Workflow (Beta) — always appends at end ──
  const workflow = useWorshipWorkflow(addMultipleSlidesAtEnd, addOfferingsSlideAtEnd);
  ...
  const handleCreatePresentation = async () => {
    ...
    try {
      ...
      // Reset workflow and pre-fill first slide with Praise & Worship
      workflow.resetWorkflow();
      await workflow.prefillFirstSlide(setSlideLines);
      setShowPrompt(false);
    } catch (err) {
  ...
  // On resume, re-evaluate the current workflow step (e.g. show modal if
  // it was at an auto/prompt step when the page was closed).
  useEffect(() => {
    if (!showPrompt && !showResumePrompt) {
      const t = setTimeout(() => workflow.fireInitialStep(), 300);
  ...
  return (
    <div className="p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        {/* Workflow beta toggle + step indicator */}
        <div className="flex items-center gap-3">
          {workflow.enabled && workflow.currentStep !== null && (
            <span className="text-[10px] font-semibold text-indigo-600 bg-indigo-50 px-2.5 py-1 rounded-full">
              Next: {workflow.currentStepDef?.label} ({workflow.currentStep + 1}/{workflow.totalSteps})
            </span>
          )}
          <button
            onClick={workflow.toggleEnabled}
            className={`flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider px-3 py-1 rounded-full border transition ${
              workflow.enabled
                ? "bg-green-50 text-green-700 border-green-300"
                : "bg-gray-50 text-gray-400 border-gray-200"
            }`}
            title={workflow.enabled ? "Disable worship flow guide" : "Enable worship flow guide"}
          >
            <span className={`w-2 h-2 rounded-full ${workflow.enabled ? "bg-green-500" : "bg-gray-300"}`} />
            Flow {workflow.enabled ? "On" : "Off"}
            <span className="text-[8px] font-normal normal-case ml-0.5 opacity-60">beta</span>
          </button>
        </div>
      </div>
      ...
      <div className="flex gap-6">
        <div className="w-1/3 h-[calc(100vh-160px)] overflow-y-auto pr-2 space-y-4">
          <SongPreview
            ...
            onAddMultipleSlides={(slideLinesArray) => {
              const ids = addMultipleSlides(slideLinesArray);
              workflow.notifySongAdded();
              return ids;
            }}
        ...
        <div className="w-2/3 flex flex-col space-y-4">
          ...
          <div className="flex justify-end gap-4">
            ...
            <button
            onClick={async () => {
              // Persist latest state before generating ZIP
              localStorage.setItem("slide-composer-session", JSON.stringify({
                presentationName,
                slides,
                currentIndex,
      ...
      {/* ── Worship Workflow Modals ── */}
      {workflow.pendingModal === "auto" && workflow.currentStepDef && (
        <WorkflowModal
          stepLabel={workflow.currentStepDef.label}
          stepIndex={workflow.currentStep}
          totalSteps={workflow.totalSteps}
          timer={workflow.currentStepDef.timer || 5}
          onConfirm={() => workflow.handleStepConfirm()}
          onSkip={workflow.handleStepSkip}
        />
      )}

      {workflow.pendingModal === "psalm" && (
        <WorkflowPsalmModal
          stepIndex={workflow.currentStep}
          totalSteps={workflow.totalSteps}
          onConfirm={(ch, vs, ve) => {
            // Build psalm slide lines
            const teluguOrd = `${ch}వ కీర్తన`;
            const engOrd = `Psalm ${ch}`;
            const lines = [
              "ఉత్తర-ప్రత్యుత్తర వాక్య పఠనం",
              "Responsive Reading",
              vs && ve ? `${teluguOrd} : ${vs}-${ve} వచనాలు` : teluguOrd,
              vs && ve ? `${engOrd} : ${vs}-${ve}` : engOrd,
            ];
            workflow.handleStepConfirm(lines);
          }}
          onSkip={workflow.handleStepSkip}
        />
      )}
